    ...
    <el-dialog title="添加推荐理由" :visible.sync="dialogVisible">
      <el-form :model="modRec" :rules="rules" ref="bookTopic">
        <el-form-item label="推荐理由" label-width="120px" prop="reason">
          <el-input type="textarea" v-model="modRec.reason" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
...
<script>
  import {reqGetSubTopicList, reqModifySubTopic, reqDelSubTopic, reqGetReason,reqAddReason} from "../../../../api/bookTopic";
    ...
    methods: {
      ...
      submitModRank(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            reqModifySubTopic(this.modRec.id, this.modRec.reason).then(response => {
                if (response.code == 200) {
                  ...
                  console.log(response)
                  reqAddReason(this.modRec.id, this.modRec.reason).then(response => {
                    console.log("nihao")
                    if (response.code == 200) {
                      this.dialogVisible = false;
                      this.$message({
