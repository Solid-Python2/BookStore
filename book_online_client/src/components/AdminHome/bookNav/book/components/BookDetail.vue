      ...
      <div class="first_tab" v-if="active===0">
        <el-form ref="book" :model="book" :rules="rules" label-width="80px">
          ...
          <el-form-item label="出版时间" prop="birthday">
            <el-col :span="12">
              <el-date-picker type="date" placeholder="选择日期" v-model="book.birthday"
  ...
  export default {
    ...
    methods: {
      ...
      onSubmit() {
        ...
        this.book.birthday = this.dateFormat(this.book.birthday)
          if(!this.book.imgSrc){
            delete this.book['imgSrc']
          }
          let form_data = new FormData();
          for (let key in this.book) {
            form_data.append(key, this.book[key]);
          }
          form_data.set('bookSort',this.book['bookSort'][1])
        if (!this.isEdit) {
          reqAddBook(form_data, headers).then(response => {
          ...
          form_data.delete('coverImg')
          reqModifyBook(form_data,headers).then(response => {
            if (response.code == 200) {
              this.$message({
                type: 'success',
                message: response.message
      ...
      handleRemove(file, fileList) {
        if (this.isEdit) {
          ...
          this.book.imgSrc=null
          console.log(this.book.imgSrc)
        }
