      ...
      <div style="margin-top: 25px">
        <el-form :inline="true" :model="search" size="small" label-width="140px">
          ...
          <el-form-item label="出版社">
            <el-select v-model="search.publish" placeholder="请选择出版社" prop="publish">
              <el-option
...
<script>
  ...
  import {
    ...
    reqModifyNew,
    reqBatchModifyPut,
    reqBatchModifyPutOff,
    reqBatchModifyRec,
    reqBatchModifyRecOff,
    reqBatchModifyNew,
    reqBatchModifyNewOff,
    reqGetSearchBookList
  } from "../../../../api/book";
  ...
  export default {
    ...
    created: function () {
      this.getPublishName();
      this.getSortList();
      if (this.$route.query.bookName || this.$route.query.author || this.$route.query.isbn || this.$route.query.publish || this.$route.query.put || this.$route.query.bookSort) {
        this.GetSearchSort(1, 5, this.$route.query.bookName, this.$route.query.author, this.$route.query.isbn, this.$route.query.publish, this.$route.query.put, this.$route.query.bookSort);
      } else {
        this.GetSort(1, 5);
      }
    ...
    methods: {
        ...
        if (this.multipleSelection.length < 1) {
          ...
          switch (this.operator) {
            case "del":
              axios({
                method: 'DELETE',
                url: 'http://localhost:8080/book/batchDel/',
                data: formData
              }).then((response) => {
              ...
              break;
            case "put":
              reqBatchModifyPut(formData).then(response => {
              ...
              break;
            case "putOff":
              reqBatchModifyPutOff(formData).then(response => {
                console.log(response);
                if (response.code == 200) {
                  this.$message({
                    message: response.message,
              ...
              break;
            case "recommend":
              reqBatchModifyRec(formData).then(response => {
                console.log(response);
                if (response.code == 200) {
                  this.$message({
                    message: response.message,
              ...
              break;
            case "recommendOff":
              reqBatchModifyRecOff(formData).then(response => {
                console.log(response);
                if (response.code == 200) {
                  this.$message({
                    message: response.message,
              ...
              break;
            case "newProduct":
              reqBatchModifyNew(formData).then(response => {
                console.log(response);
                if (response.code == 200) {
                  this.$message({
                    message: response.message,
              ...
              break;
            case "newProductOff":
              reqBatchModifyNewOff(formData).then(response => {
                console.log(response);
                if (response.code == 200) {
                  this.$message({
                    message: response.message,
      ...
      //得到搜索图书列表
      GetSearchSort(page, pageSize, bookName, author, isbn, publish, put, bookSort) {
        var data = {
          'page': page,
          'pageSize': pageSize,
          'bookName': bookName,
        ...
        reqGetSearchBookList(data).then(response => {
          if (response.code == 200) {
            this.total = response.total;
            console.log(this.total);
            this.tableData = response.bookList;
      ...
      getSortList() {
        reqGetSortList().then(response => {
          ...
          for (let i = 0; i < list.length; i++) {
            ...
            console.log(list[i]);
            this.options.push({label: list[i].sortName, value: list[i].id, children: children});
          }
        });
      },
      //得到并设置出版的下拉选择器
      ...
      Search() {
        this.$router.push({
          ...
          query: {
            ...
            isbn: this.search.isbn,
            publish: this.search.publish,
            put: this.search.put?this.search.put:'',
            bookSort: this.search.bookSort[1]?this.search.bookSort[1]:'',
          }
        })
        location.reload()
      },
