...
<template>
  <div class="content">
    ...
    <div class="box book_box">
      ...
      <div class="book_img">
        <img :src="book.coverImg" alt="" width="238px" >
      </div>
      <div class="book_buy">
        ...
        <div class="book_content">
          ...
          <el-button type="primary" icon="el-icon-shopping-cart-2" class="cartBtn" @click="addBookToCart">加入购物车
          </el-button>
        </div>
      </div>
    </div>
      ...
      <div class="book_sort">
        <div class="tab">
          ...
          <div v-for="sort in sortList" class="tab_list">
            <router-link :to="{path: '/search',query:{id:sort.id,name:sort.sortName}}">
              <div style="color: black;width: 100%">{{sort.sortName}}</div>
            </router-link>
          </div>
        </div>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      getSortList() {
        reqGetSortList().then(response => {
          if (response.code == 200) {
            console.log(response.sortResponseList)
      ...
      addBookToCart() {
        ...
        reqAddCart(this.$store.getters.getUser.username, this.book.id, 1).then(response => {
          if (response.code == 200) {
            this.$message({
