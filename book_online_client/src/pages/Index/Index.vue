                ...
                <div class="tab">
                  <div v-if="single==0">
                    <div v-for="(item,index) in options" :key="index" class="tab_list" @mouseenter="enter(index)"
                      ...
                      <div>
                        <router-link :to="{path: '/search',query:{id:item[0].value,name:item[0].label}}"><span
                          style="color: black">{{item[0].label}}</span></router-link>
                        <span> | </span>
                        <router-link :to="{path: '/search',query:{id:item[1].value,name:item[1].label}}"><span
                          style="color: black">{{item[1].label}}</span></router-link>
                        <transition name="fade">
                          <div class="tab_page" v-show="seen&&index==current">
                            <div v-for="book in item">
                              <h3>
                                <router-link :to="{path: '/search',query:{id:book.value,name:book.label}}">
                                  >{{book.label}}
                                </router-link>
                              </h3>
                              <div class="tab_page_list">
                                <el-breadcrumb separator="|">
                                  ...
                                  <el-breadcrumb-item v-for="it in book.children"
                                                      :to="{path: '/search',query:{id:it.value,name:getName(book.label,it.label)}}"
                                                      :key="it.value">
                                    {{it.label}}
                                  </el-breadcrumb-item>
                                  <el-breadcrumb-item>
...
<!--                <SpikeBox></SpikeBox>-->
            ...
            <el-tab-pane label="书单区" name="third">
              ...
              <div class="content_main_right">
                ...
                <TopicBook :bookTopicList="bookTopicList"></TopicBook>
              </div>
            </el-tab-pane>
          </el-tabs>
        </div>
...
<script>
  ...
  import TopicBook from "../../components/Index/TopicBook";
  ...
  export default {
    name: "index",
    components: {Nav, Footer, Carousel, HeadNav, GalleryBook, SpikeBox, BookBox, RecBookBox, NewBook, TopicBook},
    data() {
      return {
        recommend: "recommend",
        ...
        reqGetTopicList(page, pageSize).then(response => {
          if (response.code == 200) {
            ...
            for (let i = 0; i < list.length; i++) {
              this.bookTopicList.push({cover: list[i].cover, id: list[i].id, topicName: list[i].topicName});
            }
          }
        }).catch(err => {
          console.log(err);
          ...
          if (response.code == 200) {
            ...
            if (list.length % 2 == 0) {
              for (let i = 0; i < list.length; i = i + 2) {
                ...
                this.single = 0
                this.options.push([{
                  label: list[i].sortName,
                  value: list[i].id,
                  seen: false,
                  children: children
                }, {label: list[i + 1].sortName, value: list[i + 1].id, seen: false, children: children1}])
              }
            } else {
              ...
              if (list[list.length - 1].children != null && list[list.length - 1].children.length > 0) {
                for (let j = 0; j < list[list.length - 1].children.length; j++) {
                  children2.push({
                    label: list[list.length - 1].children[j].sortName,
                    value: list[list.length - 1].children[j].id
                  });
                }
              }
              this.options.push([{
                label: list[list.length - 1].sortName,
